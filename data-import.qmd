...
::: {.callout-note title="Alternatieven"}
...
::: {.callout-note title="Alternatief"}
...
6.  Selecteer kolom `Winnaar` en klik op de filter keuzepijl in de kolomkop. Deselecteer de optie [leeg]{.uicontrol} en klik [OK]{.uicontrol}.

7.  Kies [Sluiten en laden(groep Sluiten) \> Sluiten en laden]{.uicontrol}. De gegevens worden in een Excel tabel in een nieuw werkblad opgeslagen. Je kunt nu met de gegevens werken en de Excel map voor later gebruik opslaan.
...
Wanneer je meerdere tabellen tegelijk importeert, dan wordt er automatisch een *gegevensmodel* gemaakt met deze tabellen. Echter de **relatie tussen de tabellen** (zoals gedefinieerd in Access wordt **niet automatisch overgenomen** in het Excel-gegevensmodel.

Je moet handmatig de relaties maken. In dit geval een een-op-veel relatie van tabel Sport naar tabel Discipline via het veld Sport-ID. Dit gaat als volgt.

5.  Kies [Power Pivot \> Beheren \> Diagramweergave (groep Weergave)]{.uicontrol}.

6.  Sleep veld `Sport-ID (tabel Sport)` op veld `Sport-ID (veld Discipline)`.
...
Het gegevensmodel uit de voorgaande paragraaf wordt nu gebruikt om de gegevens te analyseren met een draaitabel.

::: {#prp-imp-access}
Analyse met draaitabel

Kies [Invoegen \> Draaitabel \> Vanuit Gegevensmodel]{.uicontrol} (zie @fig-olympic-draaitabelmaken).
...
Gebruik de draaitabel om de volgende vragen te beantwoorden.

1.  Hoeveel disciplines waren er in het beginjaar van de zomerspelen? `r fitb("10", num = TRUE)`

2.  Hoeveel disciplines zijn er nu voor de zomerspelen? `r fitb("65", num = TRUE)`
...
::: {.callout-note title="JSON syntax"}
...
## Importeren uit een afbeelding {#sec-import-afbeelding}

Excel heeft een ingebouwde functie om tabellen op afbeeldingen te importeren. Deze maakt gebruik van OCR-technologie (Optical Character Recognition) en herkent de tekst en de structuur van de tabel in de afbeelding.

::: {.callout-tip title="Afbeelding bijsnijden"}
Het is aan te raden om indien nodig vooraf de afbeelding te controleren bij te snijden zodat alleen de tabel in de afbeelding staat en alle overbodige tekst verwijderd is. De herkenning van de tabel verloopt daardoor beter.
:::


:::: {#prp-imp-txtcsv}
[Hulpbestand]{.smallcaps}: `omzet_boeken.jpg`

1.  Open in Excel een nieuwe lege werkmap.

2.  Kies [tab Gegevens \> Gegevens ophalen (groep Gegevens ophalen en transformeren) \> Uit andere bronnen \> Uit afbeelding \> Afbeelding uit bestand]{.uicontrol}.

3.  Blader naar het bestand `omzet_boeken.jpg` en klik [Invoegen]{.uicontrol}. Aan de rechterkant verschijnt een paneel waarin de gegevens geanalyseerd worden.

4. Klik op [Gegevens invoegen]{.uicontrol}. De gegevens verschijnen nu in een werkblad.

::: {.callout-note title="Controleren"}
Het kan zijn dat er onderdelen zijn die niet goed kunnen worden herkend. In dat geval wordt de betreffende cel met een kleur gemarkeerd. Wanneer je vervolgens op de knop [Controleren]{.uicontrol} klikt verschijnt er een dialoogvenster waarin je de juiste waarde kunt invoeren.
:::
