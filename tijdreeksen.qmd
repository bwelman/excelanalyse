...
#| echo: false
#| fig-cap: Tijdreeks van de maandelijkse aantallen vliegtuigpassagiers per maand (1949-1960)
#| lightbox: true

 plot(AirPassengers, xlab= "Jaar", ylab = "Aantal passagiers (x 1000)", 
 	 main="Vliegtuigpassagiers per maand gedurende 1949-1960")
```

In @fig-airpassengers (klik erop voor een groter beeld) zie je onmiddellijk een paar zaken:
...
```{mermaid}
%%| label: fig-ts-componenten
%%| fig-cap: Componenten van een tijdreeks.

flowchart TD
  T(Tijdreeks) --> TC[Trend<br/>component]
  T --> SC[Seizoen<br/>component]
  T --> CC[Cyclische<br/>component]
  T --> RC[Random<br/>component]
```
...
::: {.callout-note title="Weegfactoren"}
